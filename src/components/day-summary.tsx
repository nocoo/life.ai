...
export function DaySummaryCard({ query }: { query: DaySummaryQuery }) {
	...
	return (
		<LayerCard className="story-summary story-card" data-ai-summary="">
			<StoryCardInfo
				label="AI 总结"
				notes={[
					"以 GPS 和消费备注为生活主线，结合天气与健康；电脑活动、文章创作和 GitHub 在本模块内单独展开。",
			...
			/>
			<LayerCard.Header>
				<StoryCardHeading icon={Sparkles} title="AI 总结" />
			</LayerCard.Header>
			<LayerCard.Body className="space-y-4">
				{loading ? (
					...
					<>
						...
						{summary?.sections ? (
							<div className="story-summary-sections">
								{SECTIONS.map(({ key, title, icon, className, note }) => {
									...
									return (
										<Collapsible
											...
											className={`story-summary-section ${className}`}
