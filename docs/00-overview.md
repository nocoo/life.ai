...
每日页以 24 小时时间主干组织阅读，身体与现场在左，运动、行迹、收支和生活片刻在右。高频记录按小时与来源归组，跨小时事件用持续标记连接。地图随行迹展开，累计和 AI 总结放在日末；移动端按时间顺序合为单列。来源筛选作用于时间线、地图和数值累计；AI 总结始终读取所选本地日期的全部来源，由用户手动生成并保存。数值按完整事件集合汇总，AI 叙事使用有界采样，尊重原始时间精度。详见 [12 每日视图与 AI](12-daily-view.md) 和 [13 时间线设计](13-story-timeline.md)。
