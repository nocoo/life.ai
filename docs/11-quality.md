...
AI 链路使用每轮启动的回环 fixture，支持 OpenAI Responses 与 Anthropic Messages 协议、失败、空输出和延迟。加密密钥与 fixture token 每轮生成；local 不绑定 Workers AI。L2 覆盖全部 20 个方法/路径契约，包含数据目标、概览、Footprint 整日导入，以及 AI 设置、连接测试和总结的认证、保存、过期与并发语义。每日总结的 L1 用独立内存 SQLite 执行真实租约 SQL；L3 验证 logo 坐标、头像回退、地图、每日概览和 AI 日期切换竞态。
