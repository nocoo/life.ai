...
- Product routes `/`, `/imports`, `/connect`, `/settings/ai` and a Data Management partition containing `/data` and `/data/footprint`, plus a not-found page. No `/login`.
...
- Import of Apple Health XML, Pixiu CSV and journal JSON/NDJSON through `/api/imports`. Footprint GPX uses its dedicated daily-package API and page, with Basalt PageHeader, FileDropzone, UploadQueue, StatStrip and preview Table.
...
Thin Views are exercised by Playwright. L3 checks the exact logo x/y coordinates, avatar fallback, mobile navigation, maps, source filtering, AI settings, summaries and date-switch races. Dense scenes cover hourly map placement, repeated-area collapse, centered speed markers, solar ordering, lazy record tabs, bounded pagination, raw values, UTC detail and responsive accessibility. Model/Service/ViewModel coverage belongs to L1. Current release results are recorded in [16 Daily context and record tabs](16-daily-context-and-record-tabs.md); earlier evidence remains in docs 12, 13 and 15.
