...
日期：2026-09-14。状态：**1.3.0 已部署，完整 Footprint 数据已导入生产 D1，全量保真、重复导入和每日地图核验通过**。完整生产结果在本文末尾记录；下方保留选型测量与最终契约。

后续的 Basalt 导入界面整理、Recharts 数据概览和每日原始记录表格见 [16 每日环境与记录页签](16-daily-context-and-record-tabs.md)，沿用本文的数据存储与导入契约。
...
实验脚本和完整结果保存在本机 `/tmp/life-data-management-plan-20260914/`；原始存储实验位于 `/tmp/life-gpx-analysis-20260914/`。上表是本地候选表实测；最终生产 D1 包含正式迁移的元数据和索引，物理占用见文末。
...
- 完整原档核心核对：670,191 个点、1,625 UTC 日的六项数值与独立 Python/SQLite 参考数据逐点一致。最终正文 27,719,570 bytes，最大正文 148,794 bytes，最大单日完整 envelope 149,088 bytes。两次生产导入完成后均再次核验全部数据。

## 生产发布与完整导入核验

2026-09-14 部署 1.3.0，Worker 版本 `4dd3c525-f74f-4c5d-a9b5-5f1448c66743`，迁移 `0003_provider_days.sql` 已应用到 D1 `life`。两个域名的 `/api/live` 均返回 HTTP 200、版本 1.3.0 和 `database: ok`；应用及数据接口受 Access 保护，ingest 域名对页面、读取、导入和 Connect 管理接口返回 404。

完整输入是本机 `Downloads/backUpData-all.gpx`，160,076,209 bytes，SHA-256 为 `cec2100a652808b8033352798e714916c675772a58be53159d642ab8238d9209`。使用共享 CLI，通过已授权的 `dev:prod` 绑定写入生产 D1；服务端返回的目标确认是 `production`。

| 生产实测指标 | 结果 |
| --- | ---: |
| GPS 原始点数 | **670,191** |
| 日包数据行 / 不重复 UTC 日期 | **1,625 / 1,625** |
| 日包声明点数 / JSON 实际点数 | **670,191 / 670,191** |
| 声明正文大小 / JSON 实际 UTF-8 字节数 | **27,719,570 / 27,719,570** |
| 旧 Footprint 逐点事件行 | **0** |
| Footprint 来源 / 统计状态 / 导入会话元数据行 | **1 / 1 / 1** |
| 完整 D1 物理占用，含页、索引及元数据 | **29,437,952 bytes，约 29.44 MB / 28.07 MiB** |

数据 UTC 范围为 `2021-11-26T13:19:17Z` 至 `2026-09-13T21:53:10Z`。覆盖天数只统计有原始记录的日期，不补造空白日。

首次全量导入完成 1,625 日、670,191 点，全部为新增日包；CLI 全程耗时 42.13 秒。随后再次提交完整文件，52 个批次成功，结果为 **0 新增、0 更新、1,625 内容不变**。第二次导入后，全部日键、点数、内容哈希和日包 `updated_at` 与第一次快照相同；统计 revision 仍为 1,625，仅最近导入时间与会话回执按契约更新。

完整性校验独立于导入器：每次导入后通过生产 API 分窗口读取全部日包，与另一套 Python/SQLite 原档解析结果逐点比较时间、经纬度、海拔、速度和方向六项值，并重算哈希、字节数、每小时摘要及时间边界。全部 670,191 点一致，无缺失或额外重复；另用生产 SQL 核验行数、JSON 点数、正文大小和元数据状态。

### 查询计量与成本

生产 SQL 的 D1 `meta` 实测：读取最密集本地日所需的两个 UTC 日包，索引查询计 **3 rows_read、0 rows_written**；读取已有 provider 总量计 **1 rows_read、0 rows_written**。这些是对应查询的计量，不是整个页面全部请求的合计。完整聚合校验扫描 1,625 行，日常换日无需扫描全部点或全部日期。

最终物理大小取生产 SQL `meta.size_after=29437952`，已包含覆盖日历缓存；早先 `d1 info` 的 29,368,320 bytes 是缓存写入前的快照。按 Paid 超额存储单价 $0.75/GB·月计算，这个大小对应约 **$0.022/月** 的边际存储成本；额度内不会产生相应超额费用。本次未核实账户剩余额度，也未把有延迟的滚动 24 小时写入指标当作此次导入账单。

### 真实浏览器与每日地图

Google Chrome 152.0.7977.83 通过 Caddy `https://life.dev.hexly.ai` 读取生产 D1。概览显示正确的点数、日包行数与大小，覆盖日历的 1,625 个日期链接可跳转到每日视图。

| 上海本地日期 | 窗口内 GPS 点数 | 相交 UTC 日包 |
| --- | ---: | ---: |
| 2021-11-26 | 1 | 1 |
| 2024-08-03 | 3,526 | 2 |
| 2026-09-14 | 401 | 1 |

三个日期均显示 24 小时刻度、匹配的可见记录数和实际绘制的轨迹，OpenStreetMap 瓦片成功加载；在 2560×1440 下无横向溢出，无页面脚本错误。已目视检查完整路线和首尾标记，地图无需额外 key。

发布日志、两次导入回执、独立核验、SQL 计量与截图保存在本机私密目录 `/tmp/life-data-implementation-20260914/`；原始 GPX、坐标明细、迁移前备份及认证信息未提交到 Git。
