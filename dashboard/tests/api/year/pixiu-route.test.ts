...
describe("GET /api/year/pixiu", () => {
  beforeAll(() => {
    ...
    db.exec(`
      ...
      values 
        ...
        ('pixiu', '2025-02-16 18:00', 'Transport', 'Taxi', 0, 25, 'CNY', 'Alipay', 2025),
        ('pixiu', '2025-03-20 10:00', 'Income', 'Salary', 10000, 0, 'CNY', 'Bank', 2025);
      
      insert into pixiu_day_agg (source, day, income, expense, net, tx_count)
      values 
        ('pixiu', '2025-01-15', 0, 35, -35, 1),
        ('pixiu', '2025-02-16', 0, 25, -25, 1),
        ('pixiu', '2025-03-20', 10000, 0, 10000, 1);
      
      insert into pixiu_month_agg (source, month, income, expense, net, tx_count)
      values 
        ('pixiu', '2025-01', 0, 35, -35, 1),
        ('pixiu', '2025-02', 0, 25, -25, 1),
        ('pixiu', '2025-03', 10000, 0, 10000, 1);
      
      insert into pixiu_year_agg (source, year, income, expense, net, tx_count)
      values ('pixiu', 2025, 10000, 60, 9940, 3);
    `);
  ...
  it("should return 400 if year is missing", async () => {
    const request = new NextRequest("http://localhost/api/year/pixiu");
  ...
  it("should return 400 if year format is invalid", async () => {
    const request = new NextRequest(
      "http://localhost/api/year/pixiu?year=25"
    );
    const response = await GET(request);
    const data = await response.json();
  ...
  it("should return transformed data for valid year", async () => {
    ...
    expect(data.data.daysWithData).toBe(3);
    expect(data.data.totalIncome).toBe(10000);
  ...
  it("should return empty data for year with no records", async () => {
    const request = new NextRequest(
      "http://localhost/api/year/pixiu?year=2020"
    );
    const response = await GET(request);
    const data = await response.json();
